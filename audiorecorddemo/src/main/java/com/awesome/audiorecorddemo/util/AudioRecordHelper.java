...
public class AudioRecordHelper {
    ...
    private AudioRecord initAudioRecord() {
        // 采样频率,用于适应不同手机情况: new int[]{44100, 22050, 11025, 8000}
        int rate = 44100;
        // 录音通道：(双通道，单通道)new short[]{AudioFormat.CHANNEL_IN_STEREO, AudioFormat.CHANNEL_IN_MONO}
        short channelConfig = AudioFormat.CHANNEL_IN_MONO;
        // 编码比特率: new short[]{AudioFormat.ENCODING_PCM_16BIT, AudioFormat.ENCODING_PCM_8BIT}
...
//        File file;
//        if ((audioRecorder = initAudioRecord()) == null || (file = initTmpFile()) == null) {
//            ToastUtils.showToast(App.getInstance().getApplicationContext(), "Record init error!");
//            return null;
//        }
//        // 录音缓存大小
//        int mRecordBufSize = minShortBufferSize;
//        long endTime = 0;
...
//            // 记录开始的时间
//            while (!isDone && audioRecorder.getRecordingState() == AudioRecord.RECORDSTATE_RECORDING) {
//                int bufferReadResult = audioRecorder.read(buffer, 0, mRecordBufSize);
...
//            Application.showToast("Record-Lame init error!");
        ...
        while (!isDone) {
            ...
            ShowLogUtil.info("buffer.length="+buffer.length+",readSize="+readSize);
            // 如果读取成功
